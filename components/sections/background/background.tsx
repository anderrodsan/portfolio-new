    ...
    return (
        <div className={`snap-start flex flex-col items-center py-10 gap-5 w-full gradient1 px-5 lg:px-20 ${map ? 'h-full' : 'min-h-full'}`} id='bg'>
            ...
            <AnimatedText className='flex flex-col md:grid grid-cols-6 items-center gap-2 w-full'>
                <div/>
                <p className='flex-1 text-center col-span-4 text-sm'>A list of my education and working experience</p>
                <div className='flex justify-end'>
                    <Button size={'sm'} variant={'secondary'} onClick={() => setMap(!map)}>
                    {map ? <div className='flex-start gap-2'><List/> See List</div> : <div className='flex-start gap-2'><Map/> See Map</div>}
                    </Button>
                </div>
            ...
            {map ? (
                ...
                <div className='relative w-full flex flex-col gap-5'>
                    ...
                    {experience.map((item: Experience, index: number) => (
                        <div key={index} className={`flex w-full pl-[49px] ${item.type ? 'justify-end md:pl-[70px]' : 'md:pl-0 md:pr-[70px]'}`}>
