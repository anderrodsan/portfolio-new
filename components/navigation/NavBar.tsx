...
const NavBar: React.FC<Props> = ({}) => {
  ...
  return (
    <div className='grid grid-cols-2 md:grid-cols-3 items-center justify-center w-full px-5 lg:px-20 h-16 border-b bg-white dark:bg-black'>
      <Link className='cursor-pointer font-bold text-3xl' href={'#about'}>&R</Link>
      <div className='hidden md:flex  justify-center items-center h-full'>
        {items.map((item, index) => (
          <Link key={index} href={`#${item.path}`} className='cursor-pointer px-2 h-full flex items-center hover:font-bold'>
